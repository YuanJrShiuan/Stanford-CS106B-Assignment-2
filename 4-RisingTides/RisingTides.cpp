...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
    Vector<GridLocation> move = {
        {0,1},{0,-1},{1,0},{-1,0}
    };
    Grid<bool> submerged(terrain.numRows(),terrain.numCols(),false);
    Queue<GridLocation> q;
    for(GridLocation pt : sources){
        if(terrain.get(pt) <= height){
            submerged.set(pt,true);
            q.enqueue(pt);
        }
    }
    while (!q.isEmpty()) {
        GridLocation pos = q.dequeue();
        for(GridLocation m :move){
            GridLocation AfterMove(pos.row + m.row,pos.col + m.col);
            if(terrain.inBounds(AfterMove)){
                if(terrain.get(AfterMove) <= height && submerged.get(AfterMove) ==false){
                    submerged.set(AfterMove,true);
                    q.enqueue(AfterMove);
                }
            }
        }
